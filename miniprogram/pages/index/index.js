...
Page({
    ...
    selectMerchant: function (event) {
        ...
        var packages = packageGroups[index];
        if (!packages) {
            return;
        }
        this.setData({
    ...
    showPackageDetail: function (event) {
        var _a = event.currentTarget.dataset, title = _a.title, price = _a.price, points = _a.points;
        wx.navigateTo({
            url: "/pages/package-detail/package-detail?title=".concat(encodeURIComponent(String(title)), "&price=").concat(encodeURIComponent(String(price)), "&points=").concat(encodeURIComponent(String(points))),
        });
    },
});
